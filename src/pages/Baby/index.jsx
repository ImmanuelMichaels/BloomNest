...
import {
  NHS_SCHEDULE_VERSION, VACCINATIONS, MILESTONES, EPDS_SCREENING_WEEKS,
  getAgeConfig, getMilestoneStatus, getEpdsAlert, getAgeStageLabel,
} from '../../data/babyCareConfig';
...
export default function Baby() {
  ...
  const [achievedMilestones, setAchievedMilestones] = useState(() => loadFromStorage('babyAchievedMilestones', {}));
  const [completedVaccinations, setCompletedVaccinations] = useState(() => loadFromStorage('completedVaccinations', {}));
  
  // ── Feed Goal Slider ──
  const [feedGoalMinutes, setFeedGoalMinutes] = useState(() => {
    const saved = loadFromStorage('feedGoalMinutes', 15);
    return saved;
  });
  ...
  const vaccinations = VACCINATIONS.map(v => ({
    ...v,
  ...
  useEffect(() => { saveToStorage('babyFeeds', feeds); }, [feeds]);
  useEffect(() => { saveToStorage('babyDiapers', diapers); }, [diapers]);
  useEffect(() => { saveToStorage('babySleep', sleepLog); }, [sleepLog]);
  useEffect(() => { saveToStorage('pumpingSessions', pumpSessions); }, [pumpSessions]);
  useEffect(() => { saveToStorage('epdsCompletedWeeks', epdsCompletedWeeks); }, [epdsCompletedWeeks]);
  useEffect(() => { saveToStorage('babyAchievedMilestones', achievedMilestones); }, [achievedMilestones]);
  useEffect(() => { saveToStorage('completedVaccinations', completedVaccinations); }, [completedVaccinations]);
  useEffect(() => { saveToStorage('feedGoalMinutes', feedGoalMinutes); }, [feedGoalMinutes]);

  // EPDS screening trigger
  useEffect(() => {
    const shouldShow = EPDS_SCREENING_WEEKS.includes(babyWeeks) &&
  ...
  const handleFeed = (side) => {
    setFeeds(prev => ({
      ...
      total: prev.total + 1,
      last: side,
      [side]: prev[side] + feedGoalMinutes
    }));
  };
  ...
  if (!isBabyJourney) {
    return (
      <div className="page-pad">
        ...
        <WCard className="card-in card-in-1" style={{ textAlign: "center", padding: "var(--sp-8)" }}>
    ...
    <div className="page-pad">
      ...
      {showEPDS && (
        <div className="reveal-in" style={{
          ...
          background: "rgba(124, 58, 237, 0.15)", backdropFilter: "blur(4px)", zIndex: 2000,
          ...
          <div style={{
            ...
            <EPDSQuestionnaire onComplete={(score) => {
              ...
              const alertMsg = getEpdsAlert(score);
      ...
      {/* Age Summary Card */}
      <WCard className="card-in card-in-2" style={{ background: "rgba(124, 58, 237, 0.08)", marginBottom: "var(--gap-md)" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: "var(--gap-sm)" }}>
          ...
          <div style={{ textAlign: "right" }}>
            <Tag label={getAgeStageLabel(babyWeeks)} bg="#7C3AED" tc="#fff" />
            <MilestoneProgress />
          </div>
        </div>
      ...
      <WCard className="card-in card-in-3">
        ...
        <div style={{ marginBottom: "var(--sp-4)" }}>
          <RangeSlider
            label="Feed Duration Goal"
            value={feedGoalMinutes}
            onChange={setFeedGoalMinutes}
            min={5}
            max={40}
            step={1}
            accent="#7C3AED"
          ...
          />
          <p style={{ fontSize: 'var(--fs-xs)', color: 'var(--mt)', marginTop: 'var(--sp-1)' }}>
            💡 Drag to adjust your target feeding duration per side
          </p>
        </div>
        ...
        <div style={{ background: "var(--bll)", borderRadius: "var(--r)", padding: "var(--sp-3)", marginBottom: "var(--sp-3)" }}>
          ...
          <p style={{ fontSize: "var(--fs-xs)", color: "var(--bl)", marginTop: "var(--sp-1)", marginBottom: 0 }}>
            Recommended duration: {feedGoalMinutes} minutes per side
          </p>
        </div>
      ...
      <WCard className="card-in card-in-4">
        ...
        <div style={{ display: "flex", gap: "var(--gap-sm)" }}>
          <button onClick={() => addDiaper("Wet")} className="btn-tap choice-chip" style={{ flex: 1, padding: "var(--sp-2)", background: "var(--bll)", color: "var(--bl)", border: "none", borderRadius: "var(--r)", fontWeight: 600, cursor: "pointer" }}>
            💧 Add Wet
          </button>
          <button onClick={() => addDiaper("Dirty")} className="btn-tap choice-chip" style={{ flex: 1, padding: "var(--sp-2)", background: "var(--gdl)", color: "var(--sg)", border: "none", borderRadius: "var(--r)", fontWeight: 600, cursor: "pointer" }}>
            💩 Add Dirty
          </button>
          <button onClick={() => addDiaper("Mixed")} className="btn-tap choice-chip" style={{ flex: 1, padding: "var(--sp-2)", background: "var(--lvl)", color: "var(--lv)", border: "none", borderRadius: "var(--r)", fontWeight: 600, cursor: "pointer" }}>
            🔄 Mixed
          </button>
        </div>
      ...
      <WCard className="card-in card-in-5">
        ...
        {sleeping ? (
          <div style={{ textAlign: "center", padding: "var(--sp-4)", background: "var(--bll)", borderRadius: "var(--r)", marginBottom: "var(--sp-3)" }}>
            <div className="timer-pulse-wrap" style={{ margin: "0 auto var(--sp-2)" }}>
              <div className="timer-pulse-ring" style={{ '--pulse-color': '#7C3AED' }} />
              <span style={{ fontSize: 48 }}>😴</span>
            </div>
            <p style={{ fontSize: "var(--fs-lg)", marginBottom: "var(--sp-2)" }}>Baby is sleeping</p>
            <button onClick={stopSleep} className="btn-tap" style={{ padding: "var(--sp-2) var(--sp-4)", background: "var(--lv)", color: "#fff", border: "none", borderRadius: "var(--r)", cursor: "pointer" }}>
              Wake Up
            </button>
          </div>
        ) : (
          <button onClick={startSleep} className="btn-tap" style={{ width: "100%", padding: "var(--sp-3)", background: "var(--sgl)", color: "var(--sg)", border: "none", borderRadius: "var(--r)", fontWeight: 800, marginBottom: "var(--sp-3)", cursor: "pointer" }}>
            😴 Start Sleep Tracking
          </button>
        )}

        {sleepSummary.total > 0 && (
          <div className="reveal-in" style={{ background: "var(--gdl)", borderRadius: "var(--r)", padding: "var(--sp-3)" }}>
        ...
        {sleepLog.slice(0, 3).map(sleep => (
          <div key={sleep.id} className="reveal-in" style={{ marginTop: "var(--sp-2)", padding: "var(--sp-2)", background: "var(--card)", borderRadius: "var(--r)", border: "1px solid var(--bll)" }}>
      ...
      <div className="card-in card-in-6">
        <SectionTitle title="NHS Vaccination Schedule 💉" subtitle={`Updated ${NHS_SCHEDULE_VERSION}`} />
        <WCard>
          {nextVaccination && (
            <div className="reveal-in" style={{ background: "rgba(124, 58, 237, 0.12)", padding: "var(--sp-3)", borderRadius: "var(--r)", marginBottom: "var(--sp-4)" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "var(--gap-sm)" }}>
            {vaccinations.map((vac) => (
              <div key={vac.id} className="btn-tap" style={{
                display: "flex",
                alignItems: "center",
      ...
      <div className="card-in card-in-7">
        ...
        <WCard>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--gap-md)', marginTop: 'var(--sp-4)' }}>
            {milestones.map((milestone) => {
              const status = getMilestoneStatus(milestone, achievedMilestones, babyWeeks);
              return (
                <div key={milestone.id} className="reveal-in" style={{
                  padding: 'var(--sp-3)',
          ...
          {upcomingMilestones.length > 0 && (
            <div style={{ marginTop: "var(--sp-4)" }}>
              ...
              {upcomingMilestones.slice(0, 3).map((milestone, idx) => (
                <div key={idx} className="reveal-in" style={{ padding: "var(--sp-2)", background: "var(--bll)", borderRadius: "var(--r)", marginBottom: "var(--sp-2)" }}>
      ...
      {pumpSessions.length > 0 && (
        <div className="card-in card-in-8">
          <SectionTitle title="Pumping History 🍼" />
          {pumpSessions.slice(0, 5).map(session => (
            <WCard key={session.id} className="reveal-in" style={{ marginBottom: "var(--gap-sm)" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <div>
