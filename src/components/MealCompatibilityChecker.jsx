
import { useMemo, useState } from 'react';
import {
  searchMealSuggestions,
  resolveMealQueries,
} from '../data/foods/mealMatcher';
import {
  evaluateMealCombination,
  JOURNEYS,
  JOURNEY_LABELS,
  RESULT_TIERS,
} from '../data/foods/compatibilityEngine';

const JOURNEY_OPTIONS = [
  JOURNEYS.PRECONCEPTION,
  JOURNEYS.PREGNANCY,
  JOURNEYS.POSTPARTUM,
  JOURNEYS.BREASTFEEDING,
  JOURNEYS.MENSTRUAL,
  JOURNEYS.MENOPAUSE,
  JOURNEYS.GENERAL,
];

const TIER_STYLE = {
  [RESULT_TIERS.GENERALLY_COMPATIBLE]: { icon: '✓', className: 'tier-compatible' },
  [RESULT_TIERS.USE_CAUTION]: { icon: '⚠', className: 'tier-caution' },
  [RESULT_TIERS.AVOID_SPECIFIC_PREPARATION]: { icon: '⚠', className: 'tier-avoid-prep' },
  [RESULT_TIERS.NOT_ENOUGH_EVIDENCE]: { icon: 'ℹ', className: 'tier-unknown' },
  [RESULT_TIERS.SEEK_PROFESSIONAL_GUIDANCE]: { icon: '⚑', className: 'tier-seek-guidance' },
};

export default function MealCompatibilityChecker() {
  const [query, setQuery] = useState('');
  const [selected, setSelected] = useState([]); // array of query strings
  const [journey, setJourney] = useState('');
  const [result, setResult] = useState(null);
  const [unresolvedQueries, setUnresolvedQueries] = useState([]);

  const suggestions = useMemo(
    () => (query.trim().length >= 2 ? searchMealSuggestions(query, 6) : []),
    [query]
  );

  function addSelection(text) {
    const trimmed = text.trim();
    if (!trimmed) return;
    if (selected.some((s) => s.toLowerCase() === trimmed.toLowerCase())) return;
    setSelected((prev) => [...prev, trimmed]);
    setQuery('');
  }

  function removeSelection(text) {
    setSelected((prev) => prev.filter((s) => s !== text));
  }

  function handleInputKeyDown(e) {
    if (e.key === 'Enter') {
      e.preventDefault();
      addSelection(query);
    }
  }

  function handleCheck() {
    const queries = selected.length > 0 ? selected : (query.trim() ? [query.trim()] : []);
    if (queries.length === 0 || !journey) {
      return;
    }
    const { matched, unmatched } = resolveMealQueries(queries);
    setUnresolvedQueries(unmatched);
    if (matched.length === 0) {
      setResult(null);
      return;
    }
    const evaluation = evaluateMealCombination(matched, journey);
    setResult(evaluation);
  }

  const canCheck = (selected.length > 0 || query.trim().length > 0) && journey;

  return (
    <div className="meal-checker reveal">
      <div className="meal-checker-card">
        <h2 className="meal-checker-title">What are you craving?</h2>

        <div className="meal-checker-search">
          <input
            type="text"
            className="meal-checker-input"
            placeholder="Search for a food or meal…"
            value={query}
            onChange={(e) => setQuery(e.target.value)}
            onKeyDown={handleInputKeyDown}
            aria-label="Search for a food or meal"
          />
          {suggestions.length > 0 && (
            <ul className="meal-checker-suggestions">
              {suggestions.map((s) => (
                <li key={s.id}>
                  <button type="button" onClick={() => addSelection(s.canonicalName)}>
                    {s.canonicalName}
                    <span className="meal-checker-suggestion-country">{s.country}</span>
                  </button>
                </li>
              ))}
            </ul>
          )}
        </div>

        {selected.length > 0 && (
          <div className="meal-checker-selected">
            <span className="meal-checker-selected-label">Selected:</span>
            {selected.map((s) => (
              <span key={s} className="choice-chip choice-chip--active">
                {s}
                <button
                  type="button"
                  aria-label={`Remove ${s}`}
                  onClick={() => removeSelection(s)}
                  className="choice-chip-remove"
                >
                  ×
                </button>
              </span>
            ))}
          </div>
        )}

        <label className="meal-checker-journey-label" htmlFor="meal-checker-journey">
          Your journey
        </label>
        <select
          id="meal-checker-journey"
          className="meal-checker-journey-select"
          value={journey}
          onChange={(e) => setJourney(e.target.value)}
        >
          <option value="" disabled>
            Select your current journey…
          </option>
          {JOURNEY_OPTIONS.map((j) => (
            <option key={j} value={j}>
              {JOURNEY_LABELS[j]}
            </option>
          ))}
        </select>

        <button
          type="button"
          className="meal-checker-submit"
          disabled={!canCheck}
          onClick={handleCheck}
        >
          Check my meal
        </button>
      </div>

      {unresolvedQueries.length > 0 && (
        <div className="meal-checker-unmatched reveal">
          {unresolvedQueries.map((q) => (
            <p key={q}>
              We couldn't confidently identify "{q}" in the Femin9 Cultural Health
              Library.
            </p>
          ))}
        </div>
      )}

      {result && <CompatibilityResult result={result} />}
    </div>
  );
}

function CompatibilityResult({ result }) {
  const title = result.meals.map((m) => m.name).join(' + ');
  const overallStyle = TIER_STYLE[result.overallTier];

  return (
    <div className="meal-checker-result reveal">
      <h3 className="meal-checker-result-title">{title}</h3>
      <p className="meal-checker-result-journey">{result.journeyLabel}</p>

      <div className={`meal-checker-tier ${overallStyle.className}`}>
        <span className="meal-checker-tier-icon" aria-hidden="true">
          {overallStyle.icon}
        </span>
        {result.overallTierLabel}
      </div>

      {result.combinedNote && (
        <p className="meal-checker-combined-note">{result.combinedNote}</p>
      )}

      {result.meals.map((meal) => (
        <MealResultCard key={meal.id} meal={meal} />
      ))}

      <p className="meal-checker-disclaimer">{result.disclaimer}</p>
    </div>
  );
}

function MealResultCard({ meal }) {
  const [expanded, setExpanded] = useState(false);
  const style = TIER_STYLE[meal.tier];

  return (
    <div className="meal-checker-meal-card">
      <div className="meal-checker-meal-header">
        <h4>{meal.name}</h4>
        <span className={`meal-checker-tier-pill ${style.className}`}>
          {style.icon} {meal.tierLabel}
        </span>
      </div>

      {meal.culturalContext && (
        <p className="meal-checker-context">{meal.culturalContext}</p>
      )}

      {meal.reproductiveHealthNote && (
        <p className="meal-checker-repro-note">{meal.reproductiveHealthNote}</p>
      )}

      {meal.considerations && meal.considerations.length > 0 && (
        <div className="meal-checker-considerations">
          <h5>Preparation matters</h5>
          {meal.considerations.map((group) => (
            <div key={group.kind} className={`meal-checker-consideration-group ${group.kind}`}>
              <strong>{group.label}</strong>
              <ul>
                {group.items.map((item, i) => (
                  <li key={i}>{item}</li>
                ))}
              </ul>
            </div>
          ))}
        </div>
      )}

      <button
        type="button"
        className="meal-checker-details-toggle"
        onClick={() => setExpanded((v) => !v)}
      >
        {expanded ? 'Hide details' : 'View evidence & ingredients'}
      </button>

      {expanded && (
        <div className="meal-checker-details">
          {meal.typicalIngredients && meal.typicalIngredients.length > 0 && (
            <p>
              <strong>Typical ingredients:</strong> {meal.typicalIngredients.join(', ')}
            </p>
          )}
          {meal.evidence && (
            <p>
              <strong>Evidence level:</strong>{' '}
              {meal.evidence === 'established'
                ? 'Established general nutrition/food-safety guidance'
                : meal.evidence === 'limited'
                ? 'Limited or traditional-practice evidence, not a clinical claim'
                : 'Insufficient evidence for a specific health claim'}
            </p>
          )}
        </div>
      )}
    </div>
  );
}